...
def admin_page():
    ...
    for practice in practices:
        rows = db.execute(
            "SELECT status FROM attendance WHERE practice_id = ?",
            (practice["id"],)
        ).fetchall()
        attend = sum(1 for r in rows if r["status"] == "attend")
        absent = sum(1 for r in rows if r["status"] == "absent")
        unanswered = sum(1 for r in rows if r["status"] is None)
        summary[practice["id"]] = {"attend": attend, "absent": absent, "unanswered": unanswered}
...
def member_page():
    ...
    """, (member_id,)).fetchall()
    return render_template("index.html", page="member", practices=practices, member_name=session.get("member_name", ""))
...
def update_attendance(practice_id):
    ...
    db = get_db()
    db.execute(
        "UPDATE attendance SET status = ? WHERE practice_id = ? AND member_id = ?",
